...
> **Status: Phase 0 complete.** The application has been restructured from a single 1,325-line
> HTML file into a tested monorepo, with behaviour preserved exactly. The v1 model and its
> known defects are still in place, isolated and characterised by tests, ready to be replaced.
> See [`docs/AUDIT.md`](docs/AUDIT.md) for what is wrong and the roadmap below for the plan.
...
| `npm test`       | 103 unit tests, no browser needed (~2 s)                     |
| `npm run e2e`    | 12 Playwright smoke tests against the built app              |
...
- Only one hand is tracked, so two-handed word signs are impossible (V3).
