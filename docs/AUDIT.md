...
The v2 normaliser therefore **preserves** orientation as signal. `normalise.test.ts`
asserts rotation is preserved, which looks like the opposite of a fix until you know why.

Robustness to incidental tilt was originally bought with blanket rotation augmentation
at ±25°, chosen to stay clear of the ~90° separating K from P. That turned out to be a
compromise that served neither end. It was not enough tilt for the upright letters,
whose signers vary by far more than 25°; and because each letter was generated at a
_single_ declared angle, the coverage it produced was Gaussian — dense at that angle and
thin at the edges — with nothing at all trained between an upright letter's range and its
rotated twin's. A signer landing in that gap got `none`, and an orientation the model has
never seen does not degrade into a near miss; it degrades into silence. Measured on the
shipped pack: G held at the orientation the app's own dictionary illustrated was accepted
**0%** of the time.

Orientation is now declared per letter as a **band** (`ingest/asl_alphabet.py`), sampled
uniformly so coverage is even to its edge, and augmentation is cut to ±8° — enough for
camera tilt, small enough that the bands carry the meaning. `assert_bands_are_separable`
fails the build if augmentation ever widens a rotation-only pair's bands into each other,
which is the failure this whole arrangement exists to prevent and is otherwise silent:
both letters stay perfectly separable from every letter that is not their twin.
...
| id      | Finding                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Status                                                             |
| ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| **A1**  | **The Dictionary was 100% broken.** Images were referenced at `alphabets/a.jpg` etc.; none existed. Alphabet and number cards now show generated vector diagrams; word categories still lack artwork (Phase 4). See **A9** for the follow-on defect the first fix introduced.                                                                                                                                                                                                                                                                                                                                               | 🟢 37 generated SVG diagrams for A–Z and 0–10                      |
...
| **A9**  | **The Dictionary illustrated different letters from the ones the model was trained on.** The diagrams were generated from the fixture hand model, the classifier from the kinematic one; nobody checked they agreed. G and H were drawn pointing **169°/160°** the wrong way, R was drawn as U, K's thumb was drawn beside the fist instead of between the fingers, and E and S drew identically. Measured: signing G as illustrated was accepted **0%** of the time and read as `none` in every frame — so a user following the app's own teaching aid could not sign it, and no work on the recogniser would have helped. | 🟢 One hand model for both, pinned by `tests/test_diagrams.py`     |
| **A10** | **A rejected frame said nothing.** "The camera cannot see you" and "you are one wrist-turn from a G" rendered identically, so an orientation letter that missed looked like a letter the app could not do. The verdict already named the near miss and the reason; both went nowhere.                                                                                                                                                                                                                                                                                                                                       | 🟢 Near-miss hint with per-letter guidance                         |
...
| **A10** | Third-party webfont from `fonts.googleapis.com` — an external request in an app whose premise is that nothing leaves the device, and a hard failure on restricted networks.                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 🟢 Self-hosted via `@fontsource-variable/inter`; app works offline |
